...
func TestPreflightProbesFlagsIndividually(t *testing.T) {
	// Probing the set only reveals that something in it is bad. The whole point
	// is to keep the rest, so each flag must be asked about on its own.
	var calls int
	probe := func(_ string, flags []string) error { calls++; return nil }
	Preflight("img", []string{"-XX:a", "-XX:b", "-XX:c"}, probe)
	if calls != 3 {
		t.Errorf("probed %d times for 3 flags; flags must be tested one at a time", calls)
	}
}
...
func TestProbeArgsUnlocksFirst(t *testing.T) {
	...
	args := ProbeArgs("-XX:+UseCompactObjectHeaders")
